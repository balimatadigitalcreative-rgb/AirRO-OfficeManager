...
const positionSchema = z.object({
  lat: z.union([z.number(), z.string()]), lng: z.union([z.number(), z.string()]),
  accuracy: z.union([z.number(), z.string(), z.null()]).optional(),
...
const recordPosition = asyncHandler(async (req, res) => res.json({ data: await service.recordPosition(req.user, req.body) }));
const listPositions = asyncHandler(async (req, res) => res.json({ data: await service.listPositions(req.user) }));
...
const outstandingDeliveries = asyncHandler(async (req, res) => res.json(await service.outstandingDeliveries(req.user, req.query)));
const resolveOutstanding = asyncHandler(async (req, res) => {
  const r = await service.resolveOutstanding(req.params.id, req.body, req.user);
...
// Reset Total Stok Galon (owner) — dryRun preview (+ read-only transaksi reassurance) · commit · restore.
const resetTotalPreview = asyncHandler(async (req, res) => {
  const preview = await service.resetTotalGallon(req.body, req.user, { dryRun: true });
  const transaksi = await service.gallonResetReassurance();   // display-only; the reset never touches these
  res.json({ data: { ...preview, transaksi } });
});
const resetTotalCommit = asyncHandler(async (req, res) => { const r = await service.resetTotalGallon(req.body, req.user, { dryRun: false }); bcast('gallon', 'reset-total'); res.status(201).json({ data: r }); });
const resetTotalRestore = asyncHandler(async (req, res) => { const r = await service.restoreResetTotal(req.body.batchId, req.user); bcast('gallon', 'reset-total-restore'); res.json({ data: r }); });
const resetTotalSchema = z.object({
  mode: z.enum(['retire', 'purge']).optional(),
  counts: z.object({
    depot: z.coerce.number().int().min(0).optional(),
    armada: z.record(z.coerce.number().int().min(0)).optional(),
    rusak: z.coerce.number().int().min(0).optional(),
    customersMode: z.enum(['zero', 'enter', 'preserve']).optional(),
    customers: z.record(z.coerce.number().int()).optional(),
  }).optional(),
  note: z.string().max(500).optional(),
  confirm: z.string().max(30).optional(),
});
const resetTotalRestoreSchema = z.object({ batchId: z.string().min(3).max(60) });
// Rincian Stok Awal — per-row opening/depot management (list · bulk preview · bulk · restore).
const openingRowsList = asyncHandler(async (req, res) => res.json({ data: await service.openingRowsPanel(req.user, req.query.fleet) }));
const openingRowsBulkPreview = asyncHandler(async (req, res) => res.json({ data: await service.openingRowsBulk(req.body.ids, req.body.action, req.body, req.user, { dryRun: true }) }));
const openingRowsBulk = asyncHandler(async (req, res) => { const r = await service.openingRowsBulk(req.body.ids, req.body.action, req.body, req.user, { dryRun: false }); bcast('gallon', 'stokawal'); res.status(201).json({ data: r }); });
const openingRowsRestore = asyncHandler(async (req, res) => { const r = await service.restoreOpeningRowsBatch(req.body.batchId, req.user); bcast('gallon', 'stokawal-restore'); res.json({ data: r }); });
const openingRowsBulkSchema = z.object({ ids: z.array(z.string()).min(1).max(500), action: z.enum(['batal', 'hapus', 'pulihkan']), fleetId: z.string().max(60).optional(), note: z.string().trim().max(500).optional() });
