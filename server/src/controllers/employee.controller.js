...
const create = asyncHandler(async (req, res) => { const e = await service.create(req.body, req.user); bus.broadcast({ entity: 'employee', action: 'create', id: e.id }); res.status(201).json({ data: e }); });
const update = asyncHandler(async (req, res) => { const e = await service.update(req.params.id, req.body, req.user); bus.broadcast({ entity: 'employee', action: 'update', id: e.id }); res.json({ data: e }); });
