...
const { loginLimiter, forgotLimiter } = require('../middleware/rateLimiters');

const router = Router();

router.post('/register', validate({ body: ctrl.schemas.registerSchema }), ctrl.register);
// Login — rate-limited (max ~10 FAILED attempts / 15 min / IP) to blunt brute-force.
router.post('/login', loginLimiter, validate({ body: ctrl.schemas.loginSchema }), ctrl.login);
// Forgot password — PUBLIC (no login) + rate-limited (max 5/hour per IP) to prevent spam.
router.post('/forgot', forgotLimiter, validate({ body: ctrl.schemas.forgotSchema }), ctrl.forgot);
